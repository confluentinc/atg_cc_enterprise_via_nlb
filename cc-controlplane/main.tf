...
resource "confluent_api_key" "app-manager-kafka-api-key" {
  ...
  description  = "Kafka API Key that is owned by 'pe_app-manager' service account"
  owner {
    id          = confluent_service_account.pe_app-manager.id
    api_version = confluent_service_account.pe_app-manager.api_version
    kind        = confluent_service_account.pe_app-manager.kind
  }
...
resource "confluent_api_key" "app-consumer-kafka-api-key" {
  ...
  owner {
    id          = confluent_service_account.pe_app-consumer.id
    api_version = confluent_service_account.pe_app-consumer.api_version
    kind        = confluent_service_account.pe_app-consumer.kind
  }
...
resource "confluent_kafka_acl" "app-producer-write-on-topic" {
  ...
  principal     = "User:${confluent_service_account.pe_app-producer.id}"
  host          = "*"
  operation     = "WRITE"
  permission    = "ALLOW"
  rest_endpoint = confluent_kafka_cluster.enterprise.rest_endpoint
...
resource "confluent_api_key" "app-producer-kafka-api-key" {
  ...
  owner {
    id          = confluent_service_account.pe_app-producer.id
    api_version = confluent_service_account.pe_app-producer.api_version
    kind        = confluent_service_account.pe_app-producer.kind
  }
...
resource "confluent_kafka_acl" "app-consumer-read-on-topic" {
  ...
  resource_name = confluent_kafka_topic.orders.topic_name
  pattern_type  = "LITERAL"
  principal     = "User:${confluent_service_account.pe_app-consumer.id}"
  host          = "*"
  operation     = "READ"
  permission    = "ALLOW"
  rest_endpoint = confluent_kafka_cluster.enterprise.rest_endpoint
